...
from DataBase.DB import get_all_profiles, session, User, Profile
from DataBase.Profil_vere import get_user_preferences
from DataBase.likes import get_users_unanswered_sympathy, get_mutually_liked_users, add_like, get_pending_sympathy, \
    move_pending_to_mutual, delete_mutual_sympathy, get_unanswered_sympathy, delete_sympathy_entry
from Language.Language import load_language_texts, get_user_language
from Seting.Creat_Bot import dp, bot
from User_Keybord.User_kb import sympathies, main, create_sympathy, create_syma_keyboard, sympathy, \
    create_nextsdelete_keyboard
...
async def get_random_profiles(user_id: int, count: int):
    ...
    filtered_profiles = [
        ...
        if
        profile.id != user_id and profile.id not in unanswered_sympathy and profile.id not in mutually_liked_users and
        check_profile_against_preferences(profile, user_preferences)
    ]
    ...
    async with state.proxy() as data:
        ...
        if message.text == (texts["add_simpaty"]):
            if current_index < len(profiles):
                ...
                if sender:
                    notification_message = (texts["sympathy_received"])
                    await bot.send_message(chat_id=recipient_id, text=notification_message,
                                           reply_markup=sympathies(texts))
                    current_index += 1
                    data["current_index"] = current_index
...
async def show_sympathy_message(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        if recipient_profile:
            recipient_name_link = (f"<a href='https://t.me/{recipient_profile.telegram_username}'>"
                                   f"{recipient_profile.name}</a>")
            notification_message = f"{texts['matched_successfully_contact_user']},{recipient_name_link}"

            await bot.send_photo(chat_id=user, photo=recipient_profile.photo,
                                 caption=notification_message, reply_markup=main(texts))

            await bot.send_message(chat_id=recipient_id, text=notification_message, reply_markup=main(texts))
            current_index += 1
            data["current_index"] = current_index
...
async def process_mutual_sympathy(message, state):
    ...
    if not pending_sympathy_users:
        await message.answer(texts["no_sympathies_except_mine"], reply_markup=sympathy(texts))
        return
    ...
    if sender_profile and recipient_profile:
        ...
        notification_message_sender = f"{texts['matched_successfully_contact_user']},{recipient_name_link}"
        notification_message_recipient = f"{texts['mutual_sympathy']},{sender_profile.name}, {sender_profile.age}, " \
                                         ...
                                         f"{texts['contact_user']} {sender_name_link}"

        await bot.send_message(chat_id=user, text=notification_message_sender, parse_mode='HTML',
                               reply_markup=main(texts))

        await bot.send_photo(chat_id=recipient_id, photo=sender_profile.photo,
                             caption=notification_message_recipient,
                             reply_markup=main(texts), parse_mode='HTML')
...
async def sympathy_profiles(message: types.Message, state: FSMContext):
    ...
    if mutual_sympathy_list:
        ...
        if current_profile_index < len(mutual_sympathy_list):
            ...
            await message.answer(texts["viewed_all_profiles_with_sent_sympathy"], reply_markup=sympathy(texts))

    else:
        await message.answer(texts["no_mutual_sympathies"], reply_markup=sympathy(texts))
...
async def show_sent_sympathy_profiles(message: types.Message, state: FSMContext):
    ...
    if users_sent_sympathy:
        ...
        await message.answer(texts["no_sent_sympathy"], reply_markup=sympathy(texts))
